...
#define yield_wait_for_with_timeout(condition, timeout_time_in_milliseconds, ret) \
  ...
  {                                                                               \
    ...
    t1 = millis();                                                                \
    while (!(condition) && (millis() - t1) < timeout_time_in_milliseconds)        \
    {                                                                             \
      yield(ret);                                                                 \
    }                                                                             \
  } while (0)
...
#define yield_sleep(wait_time_in_microseconds, ret)     \
  ...
  {                                                     \
    static unsigned long t1 = 0;                        \
    t1 = micros();                                      \
    while ((micros() - t1) < wait_time_in_microseconds) \
    {                                                   \
      yield(ret);                                       \
    }                                                   \
  } while (0)
...
#define yield_sleep_ms(wait_time_in_milliseconds, ret)  \
  ...
  {                                                     \
    static unsigned long t1 = 0;                        \
    t1 = millis();                                      \
    while ((millis() - t1) < wait_time_in_milliseconds) \
    {                                                   \
      yield(ret);                                       \
    }                                                   \
  } while (0)
...
private:
  ...
  unsigned long _wait_dl_boot_ms = 3050;
  ; //give the DL app a chance to load from bootloader; needs 3 seconds
...
public:
  ...
  static const int FOODTREAT_DETECT_THRESHOLD = 23; // 60 (default), 40 (addresses empty dish issues)
