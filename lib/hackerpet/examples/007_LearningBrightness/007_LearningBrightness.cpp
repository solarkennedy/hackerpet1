...
bool playLearningBrightness()
{
    ...
    else
    {
        ...
        if ((currentLevel > 3) && (distractor_intensity > DISTRACTOR_INTENSITY_TRESHOLD_MAX[currentLevel - 1]))
        {
    ...
    if (!timeout)
    {
        if (!retryTarget && (distractor_intensity > DISTRACTOR_INTENSITY_TRESHOLD_MIN[currentLevel - 1]) && (distractor_intensity <= DISTRACTOR_INTENSITY_TRESHOLD_MAX[currentLevel - 1]))
        {
